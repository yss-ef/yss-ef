# YOUSSEF FELLAH
**Full Stack Engineer | Blockchain & AI Specialist**

> Building robust architectures and intelligent systems from the ground up.

```text
STATUS:   ON_ALTERNANCE [Full Stack @ Broker Immobilier]
LOC:      CASABLANCA, MA
STACK:    JAVA // ANGULAR // TYPESCRIPT // WEB3
```

---

### // TECHNICAL_ECOSYSTEM
<p align="left">
  <img src="https://skillicons.dev/icons?i=java,spring,nodejs,php,python,c" />
  <br/>
  <img src="https://skillicons.dev/icons?i=angular,react,ts,tailwind,flutter,solidity" />
  <br/>
  <img src="https://skillicons.dev/icons?i=aws,docker,linux,git,github,githubactions" />
  <br/>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,hibernate" />
</p>

---

### // FIELD_EXPERIENCE
*   **Broker Immobilier** | Full Stack Engineer (Alternance)
    *   *SaaS CRM Development & AI RAG Integration*
*   **HTBS Africa** | Full Stack Engineer (Intern)
    *   *ERP Optimization & Treasury Module Development*
*   **FST Errachidia** | Blockchain Developer
    *   *Decentralized E-Learning Platform Construction*

---

### // TECHNICAL_PORTFOLIOS
*   **[Spring Boot Mastery](https://github.com/yss-ef/spring-boot-portfolio)** | *Production-grade microservices and secure full-stack ecosystems (JWT, OAuth2).*
*   **[Machine Learning Showcase](https://github.com/yss-ef/machine-learning-portfolio)** | *Predictive analytics, customer intelligence, and unsupervised behavioral discovery.*
*   **[BI & ETL Pipeline](https://github.com/yss-ef/bi-etl-portfolio)** | *Enterprise data warehousing and automated KPI orchestration via Pentaho.*
*   **[IoT Systems Lab](https://github.com/yss-ef/iot-portfolio)** | *Secure telemetry (mTLS) and physics-based Industrial 4.0 simulations.*
*   **[Data Mining Discovery](https://github.com/yss-ef/data-mining-portfolio)** | *Statistical pattern discovery and retail trend optimization.*

---

### // FEATURED_TOOLS
*   **[devctl](https://github.com/yss-ef/devctl)** | *CLI Orchestrator for Spring Boot & Angular lifecycle automation.*
*   **[AWS Zabbix Monitoring](https://github.com/yss-ef/aws-zabbix-monitoring)** | *Hybrid Cloud Observability Stack architected on EC2 & Docker.*
*   **[Web3 Course Archive](https://github.com/yss-ef/decentralized-course-archive)** | *Decentralized resource management leveraging Solidity, IPFS, and React.*

---

### // ANALYTICS
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=yss-ef&show_icons=true&bg_color=16181A&title_color=E2E2E2&text_color=A0A0A0&icon_color=E2E2E2&hide_border=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=yss-ef&layout=compact&bg_color=16181A&title_color=E2E2E2&text_color=A0A0A0&hide_border=true" width="48%" />
  <br/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=yss-ef&background=16181A&hide_border=true&stroke=E2E2E2&ring=A0A0A0&fire=E2E2E2&currStreakLabel=E2E2E2&currStreakNum=E2E2E2&sideNums=E2E2E2&sideLabels=A0A0A0&dates=A0A0A0" width="97%" />
</div>

---

### // TRANSMISSION
<div align="left">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-yss--ef-16181A?style=for-the-badge&logo=linkedin&logoColor=E2E2E2)](https://linkedin.com/in/yss-ef)
[![Portfolio](https://img.shields.io/badge/Portfolio-yss--ef.io-16181A?style=for-the-badge&logo=react&logoColor=E2E2E2)](https://yss-ef.github.io)
[![Email](https://img.shields.io/badge/Email-Contact_Me-16181A?style=for-the-badge&logo=gmail&logoColor=E2E2E2)](mailto:fellahyoussef010@gmail.com)

</div>

---
<p align="center"><i>"flew too close — and touched it anyway"</i></p>

Authored by Youssef Fellah.  
Personal Project.
